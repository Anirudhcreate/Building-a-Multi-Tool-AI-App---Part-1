...
Verify when possinle; mention an alternative method briefly if relevant."""
...
def run_safe_ai_image_generator():
    ...
    img_client = InferenceClient(api_key=config.HF_API_KEY)
    ...
    def is_prompt_safe(prompt: str):
        try:
            ...
            if response.status_code != 200:
                return False, f"Filter API error: {response.status_code}"
    ...
    with st.form("img_form"):
        prompt = st.text_area("Enter image description:", height=120)
